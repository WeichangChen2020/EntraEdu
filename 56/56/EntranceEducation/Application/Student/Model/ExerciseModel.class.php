...
class ExerciseModel extends Model {
	...
	public function getNewestQuesid($openid = '', $chapid = 0, $typeid = 0) {
		...
		if($chapid != 0) {
			...
			$sql = "SELECT id FROM hs_questionbank where chapter = '$chapid' AND NOT EXISTS (SELECT quesid FROM hs_exercise where openid = '$openid' AND hs_exercise.quesid = hs_questionbank.id) limit 1";
		...
		// 此时用户按类型选择题目
		if($typeid   != 0) {
			$sql = "SELECT id FROM hs_questionbank where type = '$typeid' AND NOT EXISTS (SELECT quesid FROM hs_exercise where openid = '$openid' AND hs_exercise.quesid = hs_questionbank.id) limit 1";
	...
	public function getCurrentProgress($openid = '', $chapid = 0, $typeid = 0) {
		...
		if($chapid != 0) {
			...
			$finish_Arr = $Model->where("exer.openid='$openid' && bank.id = exer.quesid && bank.chapter=$chapid")
					->table(array('hs_exercise'=>'exer','hs_questionbank'=>'bank'))
					->group("exer.quesid") 
					->select();
	...
	public function getRankList($start = 0) { 
		...
		$sql = "SELECT openid, sum(result) FROM (SELECT DISTINCT openid,quesid,result FROM hs_exercise) P GROUP BY openid ORDER BY SUM(result) desc,openid desc LIMIT  $start,20";
		// dump($sql);	
		$Model = new \Think\Model();
		$res = $Model->query($sql);
		if (empty($res)) {
	...
	public function getPerRankList($start = 0) { 
		...
		// $sql = "SELECT openid,COUNT(result) FROM (SELECT DISTINCT openid,quesid,result FROM hs_exercise) P GROUP BY openid having COUNT(result) ORDER BY COUNT(result) desc";
		$sql = "SELECT openid, ROUND(SUM(result)/COUNT(*),2) FROM (SELECT DISTINCT openid,quesid,result FROM hs_exercise) P GROUP BY openid ORDER BY ROUND(SUM(result)/COUNT(*),2) DESC LIMIT  $start,20";
