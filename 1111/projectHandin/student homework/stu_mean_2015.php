...
a {text-decoration: none;}
    ...
    <ul>
       <?php
		...
		$id=$_GET['id'];
		$unit=$_GET['unit'];
		echo "<input type=\"button\"  onclick=\"window.location.href='homework_detail_2015.php?id=$id&unit=$unit'\" value=\"返回\">";
		...
		$get_inf=mysql_query("select * from homework where flag='$unit' and review!=''",$link) or die(a.mysql_error());
		...
		for($i=0;$i<$get_inf_num;++$i)
        {
            ...
            $get_com=mysql_query("select * from homework_comment where flag='$unit' and id='$up_id'",$link) or die(a.mysql_error());
