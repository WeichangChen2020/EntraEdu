...
//echo "</font>";
	...
	$id = $_GET['id'];
	$unit=$_GET['unit'];
	echo "<input type=\"button\"  onclick=\"window.location.href='homework.php?id=$id'\" value=\"返回\"><hr /><ul>";
	...
	echo "<br />";
	echo '作业提交状态：';
	$check_homework=mysql_query("select * from $_homework where flag='$unit' and author in (select username from $_classes where id='$id')",$link) or die(mysql_error());
	...
	$check_com=mysql_query("select * from $_homework_comment where flag='$unit' and id='$id'",$link) or die(mysql_error());
    ...
    // else
    	...
    	echo "<br />";
    	$my_work=mysql_query("select * from $_homework where flag='$unit' and uploader_id='$id'",$link) or die(mysql_error());
	...
	$check_homework=mysql_query("select * from $_homework_comment where flag='$unit' and name in (select username from $_classes where id='$id')",$link) or die(mysql_error());
	if(mysql_num_rows($check_homework)!=0)
	{
...
//echo "<a href=''>已批改完成的作业</a>";
	echo '<hr />';
	echo '<br />';
	echo "<a href='right_and_worng.php?id=$id&unit=$unit'>查看题目正确率</a><br />";
	echo "<a href='stu_mean.php?id=$id&unit=$unit'>查看心得汇总</a><br />";
	$se_st_num=mysql_query("select * from $_homework where flag='$unit' group by uploader_id",$link) or die(mysql_error());
	$up_num=mysql_num_rows($se_st_num);
	echo   "<h2> <li>第".$unit."次待批改实验报告</h2>  <ul>";  
	echo "<a href='detail.php?unit=$unit'>已批改完成的作业</a><br />";
	echo "已提交人数：$up_num<a href=\"unupdate.php?flag=$unit\">未提交名单</a><br /><hr/>";  
	$ac_work=mysql_query("select * from `$_homework` where savename!='' and flag='$unit'",$link);
	$ac_1=mysql_num_rows($ac_work);
	$ac_work=mysql_query("select * from `$_homework` where flag='$unit'",$link);
	$ac_2=mysql_num_rows($ac_work);
	echo "批改总进度：$ac_1/$ac_2   <br />";
	$result=mysql_query("select * from `$_homework` where savename='' and flag='$unit'",$link);
    ...
    {	
       ...
       { 
     	  ...
     	  echo "<a href='../student homework/ans.php?id=$id&num=$row[0]&flag=$unit'>评价</a>";
