...
//echo $uploader_id;
                       ...
                       $sql_1="select * from `$_classes` where  `id` = '$uploader_id'";
...
if($_POST)
{
    ...
    $detail=mysql_query("insert into $_home_work_detail(id,score_1,score_2,score_4,score_3,score_5,score_6,score_7,score_8,score_9,
    ...
    $num = $_POST['num'];
    $correctSql = "update $_homework set savename='$id' where `no.`='$num'";
    if(!mysql_query($correctSql, $link))
        echo mysql_error();

                       $contentStr = date("Y-m-d H:i:s",time());
                       
                     $sql="select * from `$_classes` where id='$id'";
                        	$result=mysql_query($sql,$link);
                       ...
                       $class=mysql_result($result,0,3);
                                 ...
                                 $con    =   "INSERT INTO `$_homework_comment`(`no.`,`id`,`name`,`number`,`class`,`time`,`comment`,`Homeworkscores`,`flag`) VALUES ('$num','$uploader_id','$name','$number','$class','$contentStr','$s','$Homeworkscores','$chapter')";
