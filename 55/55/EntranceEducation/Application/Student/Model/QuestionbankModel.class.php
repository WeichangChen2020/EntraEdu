...
class QuestionbankModel extends Model{
	...
	protected function getQuesChapter($cp_id = 1) {
		...
		$Chapter = M('question_chapter', 'mg_', $this->database_con);
	...
	public function getUnfishRecord($openid) { 
		...
		// 原来写的sql
		// $sql = "SELECT * FROM mg_questionbank where NOT EXISTS (SELECT * FROM mg_exercise where openid = '$openid' AND mg_exercise.quesid = mg_questionbank.id GROUP BY quesid)";


		$sql = "SELECT id FROM mg_questionbank where NOT EXISTS (SELECT quesid FROM mg_exercise where openid = '$openid' AND mg_exercise.quesid = mg_questionbank.id) limit 1" ;
...
// SELECT * FROM mg_exercise where openid = 'ohd41t3hENwHiNZTFBlbsUaB-gGw' AND result = '0' AND NOT EXISTS (SELECT * FROM mg_mistake_history where mg_exercise.quesid = mg_mistake_history.id AND mg_mistake_history.result = '1' AND openid = 'ohd41t3hENwHiNZTFBlbsUaB-gGw');
