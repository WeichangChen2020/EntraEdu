...
class ExerciseModel extends Model {
	...
	public function getNewestQuesid($openid = '', $chapid = 0, $typeid = 0) {
		...
		if($chapid != 0) {
			...
			$sql = "SELECT id FROM mg_questionbank where chapter = '$chapid' AND NOT EXISTS (SELECT quesid FROM mg_exercise where openid = '$openid' AND mg_exercise.quesid = mg_questionbank.id) limit 1";
		...
		// 此时用户按类型选择题目
		if($typeid   != 0) {
			$sql = "SELECT id FROM mg_questionbank where type = '$typeid' AND NOT EXISTS (SELECT quesid FROM mg_exercise where openid = '$openid' AND mg_exercise.quesid = mg_questionbank.id) limit 1";
	...
	public function getCurrentProgress($openid = '', $chapid = 0, $typeid = 0) {
		...
		if($chapid != 0) {
			...
			$finish_Arr = $Model->where("exer.openid='$openid' && bank.id = exer.quesid && bank.chapter=$chapid")
					->table(array('mg_exercise'=>'exer','mg_questionbank'=>'bank'))
					->group("exer.quesid") 
					->select();
	...
	public function getRankList($start = 0) { 
		...
		$sql = "SELECT openid, sum(result) FROM (SELECT DISTINCT openid,quesid,result FROM mg_exercise) P GROUP BY openid ORDER BY SUM(result) desc,openid desc LIMIT  $start,20";
		// dump($sql);	
		$Model = new \Think\Model();
		$res = $Model->query($sql);
		if (empty($res)) {
	...
	public function getPerRankList($start = 0) { 
		...
		// $sql = "SELECT openid,COUNT(result) FROM (SELECT DISTINCT openid,quesid,result FROM mg_exercise) P GROUP BY openid having COUNT(result) ORDER BY COUNT(result) desc";
		$sql = "SELECT openid, ROUND(SUM(result)/COUNT(*),2) FROM (SELECT DISTINCT openid,quesid,result FROM mg_exercise) P GROUP BY openid ORDER BY ROUND(SUM(result)/COUNT(*),2) DESC LIMIT  $start,20";
	...
	public function getMistakeData($openid) {
		...
		$quesList = $this->where($map)->field('quesid')->find();
		
		return $quesList['quesid'];
	}
	/**
	 * getMistakeNum 获取错题数量
	 * @author 陈伟昌<1339849378@qq.com>
	 * @copyright  2018-1-5 14:05 Authors
	 * @var openid
	 * @return String( 'quesid')
	...
	public function getMistakeNum($openid) {
		...
		$quesList = $this->where($map)->field('quesid')->COUNT();
